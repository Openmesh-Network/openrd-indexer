...
export async function getPrice(
  ...
  if (nativeBudget > BigInt(0)) {
    ...
    if (Number.isNaN(nativePrice)) {
      console.warn(
        `Getting price for ${coinId} returned Nan: ${JSON.stringify(
          response.data
        )}`
      )
    } else {
      const nativeAmount = formatUnits(
        nativeBudget,
  ...
  if (budget.length > 0) {
    ...
    const response = await axios.get(
      `https://api.coingecko.com/api/v3/simple/token_price/${tokenId}?contract_addresses=${tokens}&vs_currencies=usd`
    )
    await Promise.all(
      budget.map(async (erc20) => {
        try {
          ...
          if (Number.isNaN(tokenPrice)) {
            console.warn(
              `Getting price for ${tokenId}-${
                erc20.tokenContract
              } returned Nan: ${JSON.stringify(response.data)}`
            )
          } else {
          ...
          console.log(
            `Error getting token decimals ${tokenId}-${
              erc20.tokenContract
            }: ${JSON.stringify(err)}`
          )
        }
