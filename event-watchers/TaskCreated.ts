...
export async function processTaskCreated(
  event: TaskCreated,
  storage: Storage
): Promise<void> {
  ...
  await storage.users.update((users) => {
    ...
    users[disputeManager].tasks[event.chainId][taskId].push(
      TaskRole.DisputeManager
    )
  })
  ...
  await Promise.all([
    ...
    fetchMetadata(event.metadata)
      ...
      .catch((err) =>
        console.error(
          `Error while fetching task metadata ${event.metadata} (${
            event.chainId
    ...
    getPrice(chains[event.chainId], event.nativeBudget, [...event.budget])
      .then((usdValue) =>
        storage.tasks.update(
          (tasks) => (tasks[event.chainId][taskId].usdValue = usdValue)
        )
      )
      .catch((err) =>
        console.error(
          `Error while fetching usd value of ${
            event.chainId
          }-${taskId}: ${JSON.stringify(err)}`
        )
      ),
    ...
    Promise.all(
      ...
      .catch((err) =>
        console.error(
          `Error while fetching budget of ${
            event.chainId
          }-${taskId}: ${JSON.stringify(err)}`
        )
      ),
