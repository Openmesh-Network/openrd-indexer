...
export function watchSubmissionReviewed(
...
export async function processSubmissionReviewed(
  ...
  await fetchMetadata(event.feedback)
    .then((metadata) =>
      storage.tasks.update((tasks) => {
        tasks[event.chainId][taskId].submissions[
          event.submissionId
    ...
    .catch((err) =>
      console.error(
        `Error while fetching application feedback metadata ${
          event.feedback
        } (${event.chainId}-${taskId}): ${JSON.stringify(err)}`
      )
    )
}
